...
def build_overrides_dict(
    ...
    """
    Build a {key: factor} dict suitable for config.settings.CALIBRATION_OVERRIDES.

    Strategy:
      1. Per-(sport, bucket) factor when both have >= min_per_bucket trades
      2. Sport-level fallback factor when sport has enough total trades
      3. Global "default" from overall settled performance

    Factors are clamped to [0.5, 1.5] to avoid extreme leverage from small-N noise.

    Computed PER-SPORT-BUCKET by querying the trades list directly so we
    don't need extra SQL.
    """
    overrides: dict = {}
    ...
    def _clamp(f: float) -> float:
        if f is None or f != f:   # NaN
            return 1.0
        return max(0.5, min(1.5, round(f, 3)))

    # Index trades by (sport, bucket)
    by_sport_bucket: dict = {}
    for t in trades:
        sport  = t.get("sport") or "unknown"
        bucket = bucket_label(t["net_edge_pct"])
        by_sport_bucket.setdefault((sport, bucket), []).append(t)

    # 1. Per-(sport, bucket)
    for (sport, bucket), st in by_sport_bucket.items():
        if len(st) < min_per_bucket:
            continue
        wins         = sum(1 for x in st if x["outcome"] == "won")
        actual_wr    = wins / len(st)
        predicted_wr = sum(predicted_win_prob(x) for x in st) / len(st)
        if predicted_wr <= 0:
            continue
        overrides[f"{sport}__{bucket}"] = _clamp(actual_wr / predicted_wr)

    # 2. Sport-level fallbacks
    for s in sport_rows:
        if s["n"] < min_per_bucket * 2 or s["cal_f"] is None:
            continue
        overrides[s["sport"]] = _clamp(s["cal_f"])

    # 3. Global default
    n = len(trades)
    if n >= min_per_bucket:
        wins         = sum(1 for x in trades if x["outcome"] == "won")
        actual_wr    = wins / n
        predicted_wr = sum(predicted_win_prob(x) for x in trades) / n
        if predicted_wr > 0:
            overrides["default"] = _clamp(actual_wr / predicted_wr)

    return overrides


def write_overrides_to_settings(overrides: dict, settings_path: str = "config/settings.py"):
    """
    Rewrite the CALIBRATION_OVERRIDES dict block in config/settings.py.

    Locates the existing block by its sentinel comment markers and replaces
    the dict body. Falls back to safer no-op if markers aren't found.
    """
    ...
    # Build the new dict block as a string
    ts = datetime.now().strftime("%Y-%m-%d %H:%M")
    lines = [
        "CALIBRATION_OVERRIDES: dict = {",
        f"    # Auto-generated by `python scripts/calibration_report.py --update-overrides`",
        f"    # Last updated: {ts}",
        "    #",
        "    # Lookup order in risk.kelly.calibration_factor():",
        "    #   1. f\"{sport}__{bucket}\" exact match",
        "    #   2. sport (sport-only fallback)",
        "    #   3. \"default\"",
    ]
    if not overrides:
        lines.append("    # (no overrides — not enough data)")
    else:
        per_bucket = sorted(k for k in overrides if "__" in k)
        sport_only = sorted(k for k in overrides if "__" not in k and k != "default")
        if per_bucket:
            lines.append("")
            lines.append("    # Per-(sport, bucket) overrides")
            for k in per_bucket:
                lines.append(f"    {k!r:<40}: {overrides[k]:>5.3f},")
        if sport_only:
            lines.append("")
            lines.append("    # Sport-level fallbacks")
            for k in sport_only:
                lines.append(f"    {k!r:<40}: {overrides[k]:>5.3f},")
        if "default" in overrides:
            lines.append("")
            lines.append("    # Global default")
            lines.append(f"    {'default'!r:<40}: {overrides['default']:>5.3f},")
    lines.append("}")
    new_block = "\n".join(lines)

    # Replace the existing CALIBRATION_OVERRIDES block.
    # The block starts at "CALIBRATION_OVERRIDES: dict = {" and ends at the
    # matching closing "}" — find by tracking brace depth.
    import re
    start_re = re.compile(r"^CALIBRATION_OVERRIDES\s*:\s*dict\s*=\s*\{", re.MULTILINE)
    m = start_re.search(content)
    if not m:
        print(f"  ! could not find CALIBRATION_OVERRIDES block in {settings_path}")
        return False
    ...
    if end < 0:
        print(f"  ! malformed CALIBRATION_OVERRIDES block in {settings_path}")
        return False
    ...
    print(f"  Wrote {len(overrides)} override entries to {settings_path}")
    return True
...
def main():
    ...
    parser.add_argument("--update-overrides", action="store_true",
                        help="Rewrite CALIBRATION_OVERRIDES in config/settings.py "
                             "from current data (used at trade time by Kelly sizing)")
    parser.add_argument("--overrides-min-trades", type=int, default=5,
                        help="Min trades per (sport,bucket) to emit an override (default: 5)")
    ...
    if args.update_overrides:
        print(f"\n  Building CALIBRATION_OVERRIDES from {len(trades)} settled trades...")
        overrides = build_overrides_dict(
            buckets, sport_rows, trades,
            min_per_bucket=args.overrides_min_trades,
        )
        if not overrides:
            print(f"  Not enough data — need >= {args.overrides_min_trades} trades per "
                  f"(sport, bucket). Skipping.")
        else:
            print(f"  Generated {len(overrides)} override entries:")
            for k, v in sorted(overrides.items()):
                print(f"    {k:<40}  {v:.3f}")
            write_overrides_to_settings(overrides)
            print("  Restart any running scheduler/scan for new sizing to take effect.")
